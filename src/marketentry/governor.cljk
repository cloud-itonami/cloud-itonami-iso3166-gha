(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Ghana-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Ghana-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-gh-entity?
                                       true` (near-universal for Ghana
                                       ...
                                       Grounded in Ghanaian legal entity
                                       registration typically required for
                                       public awards (contracting authorities /
                                       PPA).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(TIN/統一資格/GHANEPS/PPA登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- gh-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-gh-entity? true`, INDEPENDENTLY verify
  `:has-gh-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-gh-entity?` ground truth (most Ghana public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-gh-entity? e))
                 (not (true? (:has-gh-entity? e))))
        [{:rule :gh-entity-missing
          :detail (str subject " はガーナ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- tin-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-tin? true`, INDEPENDENTLY check
  `:tin-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-tin? e))
        ...
        [{:rule :tin-unverified
          :detail (str subject " はTIN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (gh-entity-missing-violations request st)
