# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Ghana

## Classification

- Repository: `cloud-itonami-iso3166-gha`
- ISO 3166: `GHA` (Ghana)
...
- Social impact: [:ghanaian-preference-margin-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Ghanaian
  public contract
...
- registration walkthrough for GHANEPS (Ghana Electronic Procurement System), managed by the Public Procurement Authority (PPA)
- business/tax registration checklist: a business registration with the Registrar General's Department; a Tax Identification Number (TIN) and VAT registration with the Ghana Revenue Authority (GRA)
- local-content / preferential-procurement navigation: the Public Procurement Act's margin-of-preference provisions favoring Ghanaian-owned and domestically-manufactured suppliers on qualifying tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Ghanaian-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Ghana).
