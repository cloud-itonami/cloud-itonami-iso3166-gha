...
2. Register the client's intake: business type, target public function,
   prior filing history in Ghana if any.
3. Run the advisor in read-only mode against GHANEPS (Ghana Electronic Procurement System), managed by the Public Procurement Authority (PPA).
...
- named referral relationship with Ghanaian-licensed counsel or a
...
a working referral relationship with Ghanaian-licensed counsel or a
registered agent for whatever licensed representation the law of
Ghana requires for actual public-procurement filings.
